...
// 1. Kp = 15; Ki = 0; Kd = 0; encoder_move_box = 260; encoder_move_speed = 100;
...
//      }
      ...
      left_E_count = right_E_count;
    }
    else if ( ir_R_2 > ir_RF_limit ) {
      analogWrite(motorB_PWM, 85);
      analogWrite(motorA_PWM, 0);
      delay(50);
...
//
//        ir_R_2 = analogRead(PA2);
//        if ( ir_R_2 <= ir_RF_limit ) {
//          break;
//        }
//      }

      right_E_count = left_E_count;
    }
...
void calibration_mouse_move() {
  ...
  while (1) {
    ...
    if ((left_E_count + right_E_count)/2 > encoder_move_box/2) {
      analogWrite(motorA_PWM, encoder_move_speed);
      analogWrite(motorB_PWM, encoder_move_speed);
